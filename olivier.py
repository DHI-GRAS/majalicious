...
def add_parameter_files(repGipp, repWorkIn, tile):
    for fic in glob.glob(repGipp+"/*"):
        ...
        if fic.find("36JTT") > 0:
            replace_tile_name(fic, repWorkIn+'/' +
                              base.replace("36JTT", tile), "36JTT", tile)
        else:
...
if orbit is not None:
    ...
    listeProd = listeProd + \
        glob.glob(repL1+"/S2?_MSIL1C*.SAFE/GRANULE/*%s*" % (tile))
...
if not(os.path.exists(path_userconf)):
    print "create " + path_userconf
    add_config_files(repConf, path_userconf)
    ...
    if d > previous_date:
        ...
        if i == 0:
            ...
            for date_backward in dates_diff[0:nb_prod_backward]:
                ...
                os.symlink(prod_par_dateImg[date_backward], repWork +
                           "/in/"+os.path.basename(prod_par_dateImg[date_backward]))
            add_parameter_files(repGipp, repWork+"/in/", tile)
            add_DEM(repDtm, repWork+"/in/", tile)

            commande = "%s -i %s -o %s -m L2BACKWARD -ucs %s --TileId %s" % (
                maja, path_input, repL2, path_userconf, tile)
            os.system(commande)
        # else mode nominal
        else:
            # Search for previous L2 product
            for PreviousDate in dates_diff[0:i]:
                nom_courant = "%s/%s" % (repL2,
                                         nomL2_par_dateImg[PreviousDate])
                try:
                    print nom_courant
            ...
            os.symlink(prod_par_dateImg[PreviousDate], repWork +
                       "/in/"+os.path.basename(prod_par_dateImg[PreviousDate]))
            os.symlink(nomL2, repWork+"/in/"+os.path.basename(nomL2))
            os.symlink(nomL2.replace("DBL.DIR", "HDR"), repWork +
                       "/in/"+os.path.basename(nomL2).replace("DBL.DIR", "HDR"))
